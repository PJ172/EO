...
export default function OrgChartPage() {
    ...
    // Fetch base Company Structure to extract Departments for the Combobox
    const { data: companyStructure } = useQuery({
        queryKey: ['org-chart', 'structure-base'],
        queryFn: async () => {
    ...
    const departmentOptions = useMemo(() => {
        if (!companyStructure?.nodes) return [];
        return companyStructure.nodes
            .filter((n: any) => n.data?.type === 'DEPARTMENT')
            .sort((a: any, b: any) => (a.data.label || '').localeCompare(b.data.label || ''));
    }, [companyStructure]);

    const comboboxOptions = useMemo(() => {
        const defaultOption = { value: 'all', label: '🏢 SUNPLAST' };
        if (!departmentOptions) return [defaultOption];
        return [
            defaultOption,
    ...
    const { data: currentData, isLoading, refetch: refetchData } = useQuery({
        ...
        queryFn: async () => {
            if (currentView === 'DEPARTMENT' && selectedDepartmentId) {
                // If the selected ID has a prefix like "department-xxx", extract the UUID
                const rawId = selectedDepartmentId;
                const deptId = rawId.includes('-') ? rawId.slice(rawId.indexOf('-') + 1) : rawId;
    ...
    const handleSaveLayout = async (nodes: any[], configPayload?: any) => {
        ...
        try {
            ...
            await Promise.all([
                positions.length > 0 ? apiPost('/organization/positions/bulk', { positions }) : Promise.resolve(),
    ...
    const handlePositionChange = async (nodeId: string, x: number, y: number) => {
        // Individual save on drag is kept for immediate updates
        try {
            await apiPatch('/organization/position', { nodeId, x, y });
        } catch (error) {
        ...
        <div className="flex flex-col h-[calc(100vh-0rem)] bg-background overflow-hidden">
            ...
            <div className="bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm px-3 py-1.5 flex flex-wrap items-center justify-between gap-3 z-20 shrink-0">
                <div className="flex items-center gap-2 px-2">
                    <div className="h-8 w-8 rounded-lg flex items-center justify-center bg-gradient-to-br from-violet-500 to-violet-700 shadow-sm mr-1">
