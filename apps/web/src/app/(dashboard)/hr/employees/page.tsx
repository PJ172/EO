...
const EMPLOYEE_DEFAULT_COLUMNS: ColumnDef[] = [
    ...
    { key: "jobPosition", label: "Vị trí công việc" },
...
const CellContent = React.memo(({
    ...
    switch (colKey) {
        case "employeeCode":
            return <Link href={`/hr/employees/${employee.id}/edit`} className="text-xs font-semibold text-foreground hover:text-blue-600 dark:hover:text-blue-500 hover:underline transition-colors whitespace-nowrap">{employee.employeeCode}</Link>;
        case "fullName":
            return (
                <div className="flex items-center gap-3">
                    ...
                    <Link href={`/hr/employees/${employee.id}/edit`} className="text-xs font-semibold text-foreground hover:text-blue-600 dark:hover:text-blue-500 hover:underline transition-colors whitespace-normal break-words leading-tight">{employee.fullName}</Link>
                </div>
            );
        ...
        case "jobPosition": return <span className="text-xs font-medium text-blue-600 dark:text-blue-400 capitalize">{employee.currentPosition?.name || "—"}</span>;
...
export default function EmployeeListPage() {
    ...
    }, [(data as any)?.meta?.stats, debouncedSearch]);
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-0rem)] space-y-2 p-2 bg-background">
            ...
            <div className="glass shadow-sm rounded-xl p-2 animate-in fade-in slide-in-from-top-4 duration-500">
                <PageHeader
                    title="NHÂN VIÊN"
                    titleClassName="from-sky-500 to-sky-700 dark:from-sky-400 dark:to-sky-300"
                    icon={
                        <div className="h-9 w-9 rounded-xl flex items-center justify-center shadow-sm bg-gradient-to-br from-sky-500 to-sky-700">
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={jobTitleFilter || "ALL"} onValueChange={(val) => { const v = val === "ALL" ? "" : val; setJobTitleFilter(v); }}>
                                            <SelectTrigger className="h-9 rounded-lg hover:bg-muted/30 transition-colors"><SelectValue placeholder="Tất cả chức danh" /></SelectTrigger>
                                            <SelectContent className="rounded-xl">
            ...
            <div className="flex items-center justify-between mt-auto bg-card p-2 rounded-xl border border-border shadow-sm">
                ...
                <div className={selectedRows.length > 0 ? "" : "ml-auto"}>
                    <PaginationControl
                        currentPage={page}
                        totalPages={(data as any)?.meta?.totalPages || 0}
                        pageSize={limit}
                        totalCount={cachedStats.total || (data as any)?.meta?.total || 0}
                        filteredCount={debouncedSearch ? ((data as any)?.meta?.total ?? employees.length) : undefined}
