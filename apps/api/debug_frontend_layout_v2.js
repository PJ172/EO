...
function treeLayout(rootId, allNodeIds, edges, isHorizontal) {
    ...
    // EXCLUDE section child edges from DFS walk node positioning
    ...
    // After Walker finishes layout for everything EXCLUDING Section nodes:
    // Place Section nodes vertically underneath their parent department coordinates
    allNodeIds.forEach(deptId => {
        if (deptId.startsWith('department-') && result.has(deptId)) {
             ...
             let currentY = deptPos.y + NODE_H + 40; // 40px under department card
             let currentX = deptPos.x + 0; // Center aligned stack column (or offset X+50)

             sectionIds.forEach(secId => {
                 result.set(secId, { x: currentX, y: currentY });
                 currentY += NODE_H + 40; // Increment Y downwards
             });
        }
    });
...
console.log("\n=== NODE COORDINATES (HYBRID) ===");
res.forEach((pos, id) => {
    const n = allNodes.find(an => an.id === id);
    if (!id.startsWith('section-')) return; // Just print sections to keep it short
