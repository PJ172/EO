...
function moveSubtree(wm, wp, shift) {
    ...
    if (subtrees === 0) {
         console.warn(`[moveSubtree] Division by zero prevented! wp: ${wp.id}, wm: ${wm.id}`);
         return;
    }
...
function apportion(v, defaultAncestor, isHorizontal) {
    ...
    while (nextRight && nextLeft) {
        // Simple and robust apportion loop
    ...
    if (nextRight && !rightMost(vop)) vop.thread = nextRight; 
    if (nextLeft  && !leftMost(vom))  vom.thread = nextLeft;  
...
function treeLayout(rootId, allNodeIds, edges, isHorizontal) {
    ...
    edges.forEach(e => { const ch = edgeMap.get(e.source); if (ch) ch.push(e.target); });
...
dump.factories.forEach(f => {
    if (f.showOnOrgChart) {
    allNodes.push({ id: `factory-${f.id}`, name: f.name });
    if (f.companyId) allEdges.push({ source: `company-${f.companyId}`, target: `factory-${f.id}` });
    }
});
dump.divisions.forEach(d => {
    if (d.showOnOrgChart) {
    allNodes.push({ id: `division-${d.id}`, name: d.name });
    if (d.factoryId) allEdges.push({ source: `factory-${d.factoryId}`, target: `division-${d.id}` });
    else if (d.companyId) allEdges.push({ source: `company-${d.companyId}`, target: `division-${d.id}` });
    }
});
dump.departments.forEach(d => {
    if (d.showOnOrgChart) {
    allNodes.push({ id: `department-${d.id}`, name: d.name });
    if (d.divisionId) allEdges.push({ source: `division-${d.divisionId}`, target: `department-${d.id}` });
    }
});
dump.sections.forEach(s => {
    if (s.showOnOrgChart) {
    allNodes.push({ id: `section-${s.id}`, name: s.name });
    if (s.departmentId) allEdges.push({ source: `department-${s.departmentId}`, target: `section-${s.id}` });
    }
});
